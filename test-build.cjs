...
import_dotenv.default.config({ path: import_path.default.join(process.cwd(), ".env.local"), override: true });
...
function createSignedToken(userId) {
  ...
  const hmac = import_crypto.default.createHmac("sha256", secret).update(payload).digest("base64url");
...
function verifySignedToken(token) {
  ...
  try {
    ...
    const secret = process.env.BETTER_AUTH_SECRET || "tiles-default-secret";
    const expectedSignature = import_crypto.default.createHmac("sha256", secret).update(payload).digest("base64url");
...
app.get(["/api/auth/callback/google", "/api/auth/callback/google/"], async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      user = {
        id: "google_" + (profile.sub || import_crypto.default.randomBytes(6).toString("hex")),
